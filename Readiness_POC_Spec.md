...
缺失策略
- 有时长无阶段 → 按 L1 算
- 无 HealthKit 睡眠数据 → Day Mode 标记缺失，Night Mode 退回 L0 时间因子估计
...
- Engine
  - 评分规则、权重、平滑与日照校准
  - 输出 `ReadinessScore` 与 `ReadinessText`
- UI
  - SwiftUI 页面与组件
- ViewModel
...
缺失处理
- 若某子分 availability 为 `.unavailable`，则移除其权重
- 对剩余权重做归一化
...
如果有 sunrise/sunset，则进行轻微校准，幅度建议小于 10 分。
...
- Circadian
  - WeatherKit: On / Off
  - sunrise / sunset 手动输入
  - cloudCover / uvIndex 滑条（可选）
- Activity (HealthKit)
  - authStatus: 未请求 / 拒绝 / 允许
  - stepsToday / stepsLast2h
  - activeEnergyToday / activeEnergyLast2h
